...
namespace Sabio.Web.Api.Controllers
{
    ...
    public class MessageApiController : BaseApiController
    {
        ...
        {            
            ...
            try
            {                
                int id = _messageService.AddMessage(model, userId);
                ...
                else
                {
                    ...
                    _chatHub.Clients.Users(userId.ToString(), model.RecipientId.ToString()).SendMessage(message);
                }
            }
            catch (Exception ex)
            {
        ...
        [HttpGet("conversation/users/clientsorvets")]
        public ActionResult<ItemsResponse<User>> GetVetsOrClients()
        {
            int code = 200;
            BaseResponse response = null;
            int userId = _authService.GetCurrentUserId();
            bool isVet = _authService.GetCurrentUser().Roles.Equals("Vet");

            try
            {
                List<User> list = _messageService.GetVetsOrClients(userId, isVet);
